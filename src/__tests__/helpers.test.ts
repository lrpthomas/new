...
describe('Utility Functions', () => {
  ...
  describe('Pagination', () => {
    it('should return correct page of items', () => {
      ...
      const pagination = new Pagination<number>(items, 2);
  ...
  describe('UndoRedoManager', () => {
    it('should handle undo/redo operations', () => {
      const manager = new UndoRedoManager<any>();
      manager.push({ type: 'add', point: { id: 1 } });
      manager.push({ type: 'edit', point: { id: 2 } });
      expect(manager.canUndo()).toBe(true);
      expect(manager.canRedo()).toBe(false);
      const undoAction = manager.undo() as any;
      expect((undoAction as any).type).toBe('edit');
      expect(manager.canRedo()).toBe(true);
      const redoAction = manager.redo() as any;
      expect((redoAction as any).type).toBe('edit');
    });
  });
});
